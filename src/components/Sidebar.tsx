  ...
  return (
    <>      {/* Mobile Navigation Toggle - bottom right corner with matching style to top left hamburger */}
      ...
      >
        ...
        <nav className="h-[calc(100vh-120px)] overflow-y-auto scrollbar-thin scrollbar-thumb-neutral-300 dark:scrollbar-thumb-neutral-700 scrollbar-track-transparent">
          <ul className="py-2">
            {filteredNavItems.length > 0 ? (
              filteredNavItems.map((item) => (
                <li key={item.id} className="px-2 my-1">
                  <Link
                    href={`#${item.id}`}
                    onClick={() => {
                  ...
                  >
                    {item.icon && <Image src={item.icon} width={18} height={18} alt="" className={`${isClient && activeSection === item.id ? 'opacity-100' : 'opacity-70'}`} />}
                    {item.label}
                  </Link>
                </li>
              ))
            ) : (
              <li className="px-6 py-4 text-neutral-600 dark:text-neutral-400 text-sm">No results found</li>
            )}
          </ul>
        </nav>
      </div>
